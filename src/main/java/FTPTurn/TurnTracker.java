/*
 * $Id: TurnTracker.java 8380 2012-09-26 18:40:54Z uckelman $
 *
...
public class TurnTracker extends TurnComponent implements CommandEncoder, GameComponent, ActionListener, UniqueIdManager.Identifyable {
  ...
  protected static UniqueIdManager idMgr = new UniqueIdManager("FTPTurnTracker"); //$NON-NLS-1$
  ...
  protected static final String FONT_SIZE = "FTPturnFontSize"; //$NON-NLS-1$
  protected static final String FONT_BOLD = "FTPturnFontBold"; //$NON-NLS-1$
  protected static final String DOCKED = "FTPturnDocked"; //$NON-NLS-1$
  protected static final String PADDING = "FTPturnPadding"; //$NON-NLS-1$
  ...
  public TurnTracker() {
    ...
    // Create preferences
    final IntConfigurer size = new IntConfigurer(FONT_SIZE, "Turn Font", 18); //$NON-NLS-1$
    final BooleanConfigurer bold = new BooleanConfigurer(FONT_BOLD,  "Turn Font Bold", Boolean.TRUE); //$NON-NLS-1$
    final BooleanConfigurer docked = new BooleanConfigurer(DOCKED,  "Turn Docked", Boolean.TRUE); //$NON-NLS-1$
    final IntConfigurer padding = new IntConfigurer(PADDING, "Turn Extra Padding (requires restart): ", 30); //$NON-NLS-1$
    ...
    GameModule.getGameModule().getPrefs().addOption(prefTab, padding);
    ...
    padding.addPropertyChangeListener(new PropertyChangeListener() {
	    public void propertyChange(PropertyChangeEvent e) {
  ...
  protected int getPadding() {
	return ((Integer) GameModule.getGameModule().getPrefs().getValue(PADDING)).intValue();
  }
  ...
  protected void save() {
    ...
    if (!savedState.equals(getState())) {
      ...
      //BR// send turn change, but don't send chatter text if empty
      Command c;
      if (reportFormat.getText().length() > 0) {
	    c = new Chatter.DisplayText(GameModule.getGameModule().getChatter(), "* "+s);
	    c.execute();
	    c.append(new SetTurn(this, savedState));
      } else {
    	c = new SetTurn(this, savedState);
      }
  ...
  protected final int BUTTON_SIZE = 28; //BR//
  protected final int MIN_HEIGHT  = 40;
    ...
    public void setWidth(int length) {
      if (length > 0) {
        turnLabel.setMinimumSize(new Dimension(length, MIN_HEIGHT));
        turnLabel.setPreferredSize(new Dimension(length, MIN_HEIGHT));
      }
      else {
    ...
    protected void initComponents() {
      ...
      //BR// watch when our Turn Name string changes and repaint
      MutableProperty.Impl turnProp = (MutableProperty.Impl) GameModule.getGameModule().getMutableProperty("Turn Name");
	  if (turnProp != null) {
		  turnProp.addMutablePropertyChangeListener(new PropertyChangeListener() {
    ...
    public void setControls() {
      ...
      MutableProperty.Impl turnProp = (MutableProperty.Impl) GameModule.getGameModule().getMutableProperty("Turn Name");
	  if (turnProp != null) {
		String s2 = turnProp.getPropertyValue();		
		
	    s = "<html><div style=\"text-align:center\">" + s ;
	    s = s + "<br><span style=\"font-family:Arial;font-size:9px;font-weight:bold;text-align:center\">";
	    s = s + s2;
	    s = s + "</span></div></html>";
	  }
