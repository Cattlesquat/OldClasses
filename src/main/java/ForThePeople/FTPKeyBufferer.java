 ...
 */
public class FTPKeyBufferer extends VASSAL.build.module.map.KeyBufferer implements Buildable, MouseMotionListener, Drawable {
  protected Map map;
  ...
  public void mousePressed(MouseEvent e) {
    ...
    //if ((p != null) && (p instanceof Immobilized)) {
    //	ignoreEvent = true; //BR// Just reject anything with do-not-stack for a drag select
    //}
      ...
      if (!KeyBuffer.getBuffer().contains(p)) {
        ...
        if (!e.isControlDown()) {
          if (movingStacksPickupUnits || p.getParent() == null || p.getParent().isExpanded() || e.isMetaDown()
        	...
        	if (!(p instanceof Immobilized)) { //BR// reject do-not-stack
            ...
            for (int i = 0; i < s.getPieceCount(); i++) {
              ...
              if ((pp != null) && !(pp instanceof Immobilized)) { //BR// reject do-not-stack
      ...
      if (selection.contains(p.getPosition()) && !Boolean.TRUE.equals(p.getProperty(Properties.INVISIBLE_TO_ME))) {
        if (selecting) {
          ...
          if (filter == null || altSelect) {
        	if (p instanceof Immobilized) { //BR// leave out all does-not-stack units
