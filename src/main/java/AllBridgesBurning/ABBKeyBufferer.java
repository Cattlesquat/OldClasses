//BR// Changes here -- make Does-Not-Stack units unaffected by band-select

/*
 * $Id: KeyBufferer.java 7725 2011-07-31 18:51:43Z uckelman $
 *
 ...
 */
public class ABBKeyBufferer extends VASSAL.build.module.map.KeyBufferer implements Buildable, MouseMotionListener, Drawable {
  protected Map map;
  protected Rectangle selection;
  protected Point anchor;
  protected Color color = Color.black;
  protected int thickness = 3;
  protected boolean no_deselect = false;
  ...
  public void mousePressed(MouseEvent e) {
    ...
    int band_select = 0;
    if ((p != null) && (Decorator.getDecorator(Decorator.getOutermost(p), NonRectangular.class) != null)) {
      if (!e.isMetaDown()) {
        band_select = 2; //BR// Can band-select from the provinces, on non-right-click
      }
    }
    if (p != null && !ignoreEvent) {
      ...
      if (!KeyBuffer.getBuffer().contains(p)) {
        ...
        if (!e.isControlDown()) {
          if (movingStacksPickupUnits || p.getParent() == null || p.getParent().isExpanded() || e.isMetaDown()
          	...
          	//if (Decorator.getDecorator(Decorator.getOutermost(p), NonRectangular.class) == null) {   
            ...
            for (int i = 0; i < s.getPieceCount(); i++) {
              GamePiece pp = s.getPieceAt(i);
              if (pp!= null) {
               	//if (Decorator.getDecorator(Decorator.getOutermost(pp), NonRectangular.class) == null) {   
	              KeyBuffer.getBuffer().add(pp);
	            //}
              }
            }
          }
      ...
      if (Decorator.getDecorator(Decorator.getOutermost(p), NonRectangular.class) == null) { //BR// don't move giant provinces to the front
	      if (p.getParent() != null) {
	      ...
	      if (p.getParent() != null) {
		    map.getPieceCollection().moveToBack(p.getParent());
		  } else {
		    map.getPieceCollection().moveToBack(p);
		  }    	  
      }
    } else {
      band_select = 1;
    ...
    if (band_select > 0) {
      if (!e.isShiftDown() && !e.isControlDown()) { // No deselect if shift key down
	    ...
	    } else if (band_select == 2) {
	      if (p != null) KeyBuffer.getBuffer().add(p); 	
	    }    	
      } 
      no_deselect = (band_select == 2);
  ...
  public void mouseReleased(MouseEvent evt) {
    if (selection != null) {
      ...
      if (!no_deselect && !evt.isShiftDown() && !evt.isControlDown()) { 
        KeyBuffer.getBuffer().clear();
      }
      map.apply(d);
      ...
      if (selection.contains(p.getPosition()) && !Boolean.TRUE.equals(p.getProperty(Properties.INVISIBLE_TO_ME))) {
        if (selecting) {
          ...
          if (filter == null || altSelect) {
            KeyBuffer.getBuffer().add(p);   //BR// NO idea why i need to comment this out?
        	if (Decorator.getDecorator(Decorator.getOutermost(p), NonRectangular.class) != null) {
      	      if (p.getParent() != null) {
      		    map.getPieceCollection().moveToBack(p.getParent());
      		  } else {
