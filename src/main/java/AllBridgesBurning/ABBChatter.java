 ...
 */
public class ABBChatter extends VASSAL.build.module.Chatter implements CommandEncoder, Buildable {
	...
	public static class DisplayText extends Command {
		private String msg;
		private ABBChatter c;

		public DisplayText(ABBChatter c, String s) {
			this.c = c;
			msg = s;
			if (msg.startsWith("<>")) {
				msg = "&lt;(" + ABBChatter.getAnonymousUserName() + ")&gt;" + s.substring(2); // BR// HTML-friendly
																								// angle brackets
			} else {
				msg = s;
			}
	...
	public static void main(String[] args) {
		ABBChatter chat = new ABBChatter();
		JFrame f = new JFrame();
		f.add(chat);
		f.pack();
		f.setVisible(true);
