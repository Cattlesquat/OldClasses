...
public class ABBMenuDisplayer extends MouseAdapter implements Buildable {
    ...
    if (c != null) {
      ...
      for (int i = 0; i < c.length; ++i) {
        ...
        if (c[i] instanceof KeyCommandSubMenu) {
          JMenu subMenu = new JMenu(fixMenuString(c[i].getLocalizedMenuText()));
          ...
          if (strokes.contains(stroke)) {
            ...
            if (action != null) {
              ...
              if (commandName == null ||
                ...
                item = new JMenuItem(fixMenuString(c[i].getLocalizedMenuText()));
      ...
      if (p != null) {
        ...
        if (filter == null
          ...
          if (map.getView().isShowing()) {
        	...
        	//BR// Inform the piece where player clicked, if it wants to know.  
            final ChangeTracker ch = new ChangeTracker(p);
            Point pc = map.mapCoordinates(pt);
            p.setProperty("ClickedX", pc.x);
            p.setProperty("ClickedY", pc.y);
