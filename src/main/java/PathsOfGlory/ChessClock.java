...
public class ChessClock extends AbstractConfigurable implements CommandEncoder, GameComponent, ActionListener {
  ...
  public class UpdateTimerCommand extends Command {
    private String who; // who = player who is doing the reporting
    private String name; // name = whose timer is being reported
    private long elapsed; // Elapsed time from our point of view
    private long verified; // Verified time (both clients have agreed)
    private boolean ticking; // Timer currently ticking?
    private boolean restore; // Restoring save?
  ...
  protected boolean setTimerButton() {
  ...
  public static class ColorConfig implements ConfigurerFactory {
    public Configurer getConfigurer(AutoConfigurable c, String key, String name) {
      return new ColorConfigurer(key, name, ColorConfigurer.stringToColor(((ChessClock) c).getAttributeValueString(TICKING_BACKGROUND_COLOR)));
    }
  }

  public static class ColorConfig2 implements ConfigurerFactory {
    public Configurer getConfigurer(AutoConfigurable c, String key, String name) {
      return new ColorConfigurer(key, name, ColorConfigurer.stringToColor(((ChessClock) c).getAttributeValueString(TICKING_FONT_COLOR)));
    }
  }
  
  public static class ColorConfig3 implements ConfigurerFactory {
    public Configurer getConfigurer(AutoConfigurable c, String key, String name) {
      return new ColorConfigurer(key, name, ColorConfigurer.stringToColor(((ChessClock) c).getAttributeValueString(TOCKING_FONT_COLOR)));
    }
  }
}
